...
impl AudioMixer {
    ...
    pub fn set_channel_1(&mut self, source: AudioSource) {
        self.channels[0] = Some(source);
    }

    pub fn set_channel_2(&mut self, source: AudioSource) {
            ...
            if let Some(ref mut audio_source) = audio_source {
                ...
                if next_source_position > audio_source.samples.len() {
                    ...
                    next_buffer[0..remaining_samples].copy_from_slice(
                        &audio_source.samples[audio_source.position..audio_source.samples.len()],
                    );
